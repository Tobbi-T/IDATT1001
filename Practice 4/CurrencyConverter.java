...
public class CurrencyConverter {
    public static void main(String[] args) {
        ...
        System.out.printf("%s er valgt.%nØnsker du å konvertere til(1) eller fra(2) Norske kroner?%n", currencies[currencyChoice].getName());
        final boolean toNOK = scanner.nextInt() == 1;

        System.out.printf("%s er valgt.%nHvor mye ønsker du å konvertere?%n", toNOK ? "Til Norske kroner" : "Fra Norske kroner");
        final double amount = scanner.nextDouble();

        if(toNOK)
            System.out.printf("%.2f %s er %.2f Norske kroner.", amount, currencies[currencyChoice].getName(), currencies[currencyChoice].toNOK(amount));
        else
            System.out.printf("%.2f Norske kroner er %.2f %s.", amount, currencies[currencyChoice].fromNOK(amount), currencies[currencyChoice].getName());
