...
public class PropertyRegister {
     ...
     */
    public void addProperty(int munNumber, String munName, int lotNumber, int sectionNumber, float area, String ownerName, String propertyName) throws IllegalArgumentException {
        if(area == 0) {
            throw new IllegalArgumentException("Arealet til eiendommen kan ikke være 0.");
        }

        if(munName.isEmpty() || ownerName.isEmpty() || (propertyName != null && propertyName.isEmpty())) {
            throw new IllegalArgumentException("Navn må ha en tekstverdi.");
        }

        Property p = new Property(munNumber, munName, lotNumber, sectionNumber, area, ownerName, propertyName);
    ...
    public Property getProperty(int munNumber, int lotNumber, int sectionNumber) {
        ...
        for(Property q : register) {
            if(q.getMunNumber() == munNumber && q.getLotNumber() == lotNumber && q.getSectionNumber() == sectionNumber) {
                p = q;
            }
        }
